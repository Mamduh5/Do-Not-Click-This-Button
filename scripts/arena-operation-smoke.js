...
module.exports = async function checkOperations(page) {
  ...
  assert(snapshot.paused && snapshot.enemyCount === 0 && snapshot.energy === rewardEnergy && snapshot.wave === 1, "optional upgrade pause freezes automatic progression without rewarding again");
  ...
  assert((await page.evaluate(() => window.__containmentArena.getSnapshot())).wave === 2, "wave two starts automatically after resuming the countdown");
