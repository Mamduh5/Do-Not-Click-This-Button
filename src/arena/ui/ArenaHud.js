(function () {
  ...
  function createArenaHud(options) {
    ...
    function update(state, combo, scene) {
      ...
      element("Phase").textContent = paused ? copy.paused : cleared ? copy.cleared : champion ? "CHAMPION ENCOUNTER" : copy.active;
      element("WaveProgress").textContent = state.waveKills + " / " + definition.target;
      element("WaveTitle").textContent = definition.title;
      element("WaveHint").textContent = cleared ? "Next: " + next.title + " / " + next.target + " targets" + (next.champion ? " / Champion finale" : "") : champion ? "Final target. Break the crown to secure the room." : definition.hint;
      element("RewardPreview").textContent = "+" + definition.clearReward + (cleared ? " ENERGY SECURED" : " ENERGY ON CLEAR");
      ...
      element("PulseBtn").disabled = paused || cleared || !activeEnemies.length || !pulseReady;
